...
const {fn, str, rt} = require('./utils/generation');
...
function postProccess(options, extracted, res) {
    ...
    if (options.modules === 'es6') {
        prefix += `import * as joy from "${options.runtimePath.replace(/\\/g, '/')}";\r\n`;
        ...
        prefix += req('joy', options.runtimePath.replace(/\\/g, '/'));
        ...
        const depNames = ['joy'];
        const depPathes = [options.runtimePath.replace(/\\/g, '/')];
