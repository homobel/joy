...
const nodeProcessors = {
    ...
    ConditionWithExpression(node, exported, options) {
        ...
        return '(' + processNodes(expr, exported, options) + ')?' + processNodes(block, exported, options) + ':'
    },
    ...
    FnIdentifier(node, exported, options) {
        ...
        const blockRes = hasBlock ? processNodes(block, exported, options) : '';
        ...
        if (hasNamedArgs || hasBlock) {
            if (hasArgs) {
                ...
                if (hasBlock) {
                    return processNodes(id, exported, options) + '({content:' + (blockRes || str('')) + '})';
                }
                else {
...
module.exports = (ast, options) => {
    ...
    return postProcess(options, extracted, processNodes(ast, extracted, options));
};
