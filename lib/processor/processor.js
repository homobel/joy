...
const {fn, str, rt, notEmpty} = require('./utils/generation');
const {handle} = require('./utils/handle-ast-node');
...
const nodeProcessors = {
    ...
    Unary(node, exported, options) {
        ...
        return node.get('operator') + processNodes(node.childs(), exported, options);
    },
